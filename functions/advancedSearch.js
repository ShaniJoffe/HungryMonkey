...
new Promise((resolve,reject) => {
	esClient.search({//check if dish exists
				...
				"bool":{
					"must": [
						{"nested": {
							...
							"inner_hits": {
							//	explain:true,
								_source: ['menu.dish_name','menu.dish_description','menu.dish_price','menu.imgUrl','menu.dish_id_inRest','menu.dish_cat']//keys of menu object which will be presented
							},
							"query": {
								"bool": {
									"must":[
		...
		"filter" : // query to filter results by distance and Kosher user had input
		{
			"bool":{
				"must":[
					{
