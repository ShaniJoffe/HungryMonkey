...
exports.getFavorites = (id)=>// gets  users favorites by id
		...
		client.smembers(key,function(err,result){
			if(!err){
				...
				esClient.search({//check if exists
					...
					type: 'restaurants',
					_source: ['rest_name','rest_description','rest_location','Kosher','rest_address'],// those are the fields which we return
					body :{
						"query": {
							"bool":{
								"must": [
									{"nested": {
										...
										"inner_hits": {
										//explain:true,
											_source: ['menu.dish_name','menu.dish_description','menu.dish_price','menu.imgUrl','menu.dish_id_inRest']// those are the fields which we return
										},
										"query": {
											 "bool" : {
												"should" : [
